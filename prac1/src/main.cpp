...
int main(int argc, char* argv[]) {
  ...
  cv::Mat imgThreshold;   // threshold image
  cv::Mat imgAdaptive;    // adaptive image
  ...
  while (charCheckForESCKey != 27 && webCam.isOpened()){    // loop until ESC key is pressed or webcam is lost
    ...
    cv::threshold(imgGrayScale, imgThreshold, threshold, 255, cv::THRESH_BINARY);
    cv::adaptiveThreshold(imgGrayScale, imgAdaptive, 255, cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY, blockSize, 2);
    ...
    cv::namedWindow("imgThreshold", cv::WINDOW_AUTOSIZE);
    cv::namedWindow("imgAdaptive", cv::WINDOW_AUTOSIZE);
    ...
    cv::createTrackbar("Threshold", "imgThreshold", &threshold, 255);
